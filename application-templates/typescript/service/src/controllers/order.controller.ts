 ...
 */
const create = (item: Order): ControllerResult => {
  logger.info(`Running API Extension for Order Update (ID: ${item.id})`);
  return;
};
 ...
 */
const update = (item: Order): ControllerResult => {
  logger.info(`Running API Extension for Order Update (ID: ${item.id})`);
  return;
};
 ...
 */
export const orderController = (action: ExtensionAction, resource: Order) => {
  switch (action) {
    case 'Create':
